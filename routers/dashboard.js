		...
		try {
			const tokenResponseData = await request('https://discord.com/api/oauth2/token', {
				...
				body: new URLSearchParams({
					...
					redirect_uri: `${settings.fullredirecturl}`,
            ...
            var foundHost = tempStorage.find(i => i.hostname == req.ip);
...
router.get('/dash/end', async function(req, res) {
    ...
    try {
        const tokenResponseData = await request('https://discord.com/api/oauth2/token/revoke', {
			method: 'POST',
			body: new URLSearchParams({
        ...
        for (i = 0, j = tempStorage.length; i < j; i++) {
            cur = tempStorage[i];
...
router.get('/dash/redirect', function(req, res) {
    res.redirect(`https://discord.com/oauth2/authorize?client_id=${settings.clientid}&response_type=code&redirect_uri=${settings.redirecturl}&scope=guilds+identify`);
});
